...
def logistic_Regression(X_train, X_test, y_train, y_test):
    ...
    print(f"\nPredicciones: ")
    for i, actual_value in enumerate(np.unique(y_test)):
        for j, predicted_value in enumerate(np.unique(y_test)):
            if confu_matrix[i, j] > 0:
                print(f"Prediccion: {predicted_value}, Actual: {actual_value}, Count: {confu_matrix[i, j]}")

    # Graficar las métricas
    metrics_names = ['Accuracy', 'Precision', 'Recall', 'Specifity', 'F1 Score']
    metrics_values = [accuracy, precision, recall, specificity, f1]

    plt.bar(metrics_names, metrics_values, color=['blue', 'mediumspringgreen', 'red', 'plum', 'turquoise'])
